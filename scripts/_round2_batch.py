#!/usr/bin/env python3
"""Round 2 — coffee + kitchen ASINs only."""
...
# Pick fresh ASINs past the ones that 404'd
BATCH = [
    ("coffee", "B0C4SZDMGX"),
    ("coffee", "B0CX23V2Y2"),
    ("coffee", "B09QGJQV8Q"),
...
for cat, asin in BATCH:
    if cat in targets:
        needed = targets[cat] - sum(1 for c,_,_ in results if c == cat)
        if needed <= 0:
            print(f"\n[{cat.upper()}] Target reached — skipping remaining {cat} ASINs")
            continue
...
print(f"\n{'='*60}")
print(f"ROUND 2 COMPLETE: {success} scraped, {skipped} skipped")
