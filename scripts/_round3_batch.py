#!/usr/bin/env python3
"""Round 3 — coffee + kitchen ASINs that verified HTTP 200 directly."""
...
# These ASINs were verified to return HTTP 200 from our location
BATCH = [
    ("coffee", "B08GJHYNQF"),
    ("coffee", "B0BN6C8XWK"),
    ("coffee", "B0C9YBRN4H"),
...
skipped = 0
results = []
targets = {"coffee": 4, "kitchen": 3}

for cat, asin in BATCH:
    needed = targets.get(cat, 99) - sum(1 for c,_,_ in results if c == cat)
    if needed <= 0:
        print(f"\n[{cat.upper()}] Target reached — skipping remaining")
        continue
...
print(f"\n{'='*60}")
print(f"ROUND 3 COMPLETE: {success} scraped, {skipped} skipped")
